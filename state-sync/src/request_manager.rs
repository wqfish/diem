...
pub struct RequestManager {
    ...
    // the more networks this node will send multicast requests to. The lower the network level, the
    // less networks will receive requests. Ordering is defined by NetworkId.
...
impl RequestManager {
    ...
    pub fn pick_peers(&mut self) -> Vec<PeerNetworkId> {
        ...
        for (network_level, (peers, weighted_index)) in eligible_peers.iter() {
            ...
            if !chosen_peers.is_empty() && *network_level >= self.multicast_network_level {
                self.update_multicast_network_level(network_level.clone(), None);
                break;
            }
        }
